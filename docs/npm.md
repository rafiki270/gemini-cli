...
This monorepo contains two main packages: `@unlikeotherai/gemini-cli` and
`@unlikeotherai/gemini-cli-core`.

## `@unlikeotherai/gemini-cli`
...
`@unlikeotherai/gemini-cli-core`. This means that whether a user installs the
package with `npm install -g @unlikeotherai/gemini-cli` or runs it directly with
`npx @unlikeotherai/gemini-cli`, they are using this single, self-contained
executable.
...
- **Simplified script execution**: You can run scripts in any package from the
  ...
  `npm run build --workspace @unlikeotherai/gemini-cli`.
